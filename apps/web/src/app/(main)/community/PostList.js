      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-wrap gap-2">
            {[
              { key: 'all', name: '전체' }, { key: 'notice', name: '공지사항' },
              { key: 'free', name: '자유게시판' }, { key: 'question', name: '질문' },
              { key: 'info', name: '정보공유' }, { key: 'trade', name: '거래' }
            ].map(category => (
              <button
                key={category.key}
                onClick={() => setCurrentCategory(category.key)}
                className={`category-filter-btn px-3 py-2 rounded-lg text-sm font-medium transition ${currentCategory === category.key ? 'active bg-purple-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                {category.name}
              </button>
            ))}
