...
function Comment({ 
    ...
    const authorDisplayKey = comment.isAnonymous ? `anon_${comment.authorId}` : comment.authorId;
    ...
    return (
        <div style={{ marginLeft: `${depth * 32}px` }} className={`mt-4 ${depth > 0 ? 'border-l-2 border-gray-200 pl-4' : ''}`}>
            ...
            {replyingTo === comment.id && (
                <ReplyInputForm
                    authorNickname={comment.isAnonymous ? `익명${authorIndex}` : comment.authorNickname}
