...
export default function InformationPage() {
    ...
    useEffect(() => {
        ...
        const fetchShuttle = async () => {
            ...
            try {
                const response = await apiClient.get(`/shuttles/${encodeURIComponent(user.university)}`);
    ...
    useEffect(() => {
        const filtered = shuttleData.filter((route) => {
            if (!route || !route.routeName) return false;
                                ...
                                <div className="mt-4 pt-4 border-t text-sm text-gray-600">
                                    <div className="flex items-center justify-between">
                                        <span>
                                            ...
                                            출발: {schedules[0].departureLocation}
                                        </span>
                                        <span>
                                            <i className="fas fa-flag-checkered mr-1 text-red-500"></i>
                                            도착: {schedules[0].arrivalLocation}
                                        </span>
                                    </div>
                                </div>
                            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <p className="text-xl text-gray-600">
                        {user.university
                            ? `${user.university} 학식 메뉴와 셔틀버스 시간표를 한눈에 확인하세요`
                ...
                {activeTab === "meal" && (
                    <div>
                        ...
                        <div className="text-center py-16 bg-white rounded-xl shadow-sm">
                            ...
                            <h3 className="text-xl font-semibold text-gray-600 mb-2">학식 정보가 없습니다</h3>
                            <p className="text-gray-500">등록된 학식 정보가 없습니다.</p>
                        </div>
                        {/* --- 수정된 부분 끝 --- */}
                    </div>
                )}
