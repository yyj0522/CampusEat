...
const ChatView = ({ user, meeting, socket, onKickUser, onLeaveMeeting, onAcknowledgeKick }) => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            if (diff <= 0) { setTimeRemaining("모임 종료"); setIsExpired(true); clearInterval(interval); return; }
    ...
    useEffect(() => {
        if (!socket || !meeting?.id || isKicked) {
            setMessages([]);
            return;
        };

        apiClient.get(`/gatherings/${meeting.id}/messages`)
    ...
    if (!meeting) {
        return <div className="flex-1 flex items-center justify-center bg-gray-100 h-full rounded-r-lg"><p className="text-gray-500">왼쪽에서 참여중인 모임을 선택하세요.</p></div>;
    }
    ...
    if (isKicked) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center bg-gray-100 h-full rounded-r-lg p-4 text-center">
                ...
                <h3 className="text-xl font-semibold text-gray-800">해당 모임에서 강퇴당하셨습니다.</h3>
                <p className="text-gray-500 mt-2 mb-6">더 이상 이 모임의 채팅에 참여할 수 없습니다.</p>
                <button 
    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-white rounded-r-lg border-l border-gray-200 relative">
             {isExpired && ( <div className="absolute inset-0 bg-black bg-opacity-60 flex flex-col items-center justify-center z-20 text-white text-center p-4"><h3 className="text-2xl font-bold mb-4">모임이 종료되었습니다.</h3><p>이 채팅방은 더 이상 활성화되지 않습니다.</p></div> )}
            <header className="p-4 bg-white text-gray-800 font-bold border-b flex justify-between items-center flex-shrink-0">
                <div className="flex flex-col"><span className="truncate font-semibold text-base">{meeting.title}</span><span className="text-xs font-mono text-red-500 font-normal">{timeRemaining} 남음</span></div>
                <div className="flex items-center gap-4">
                    ...
                    {user.id !== meeting.creator.id && (
                        <button onClick={() => onLeaveMeeting(meeting)} className="text-gray-500 hover:text-red-500 transition" title="모임 나가기">
...
export default function MeetingPage() {
    ...
    const fetchGatherings = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const myGatheringsRes = await apiClient.get('/gatherings', { params: { type: 'myMeetings' } });
            setMyGatherings(myGatheringsRes.data);

            if (meetingType !== 'myMeetings') {
                const allGatheringsRes = await apiClient.get('/gatherings', { params: { type: meetingType } });
    ...
    useEffect(() => {
        if (!authLoading && user) {
            ...
            newSocket.on('updateGathering', (updatedGathering) => {
                setMyGatherings(prev => prev.map(g => g.id === updatedGathering.id ? updatedGathering : g).filter(g => new Date(g.datetime) > new Date()));
                setGatherings(prev => prev.map(g => g.id === updatedGathering.id ? updatedGathering : g));
            ...
            const handleRemoved = (data) => {
                showAlert(data.title ? `'${data.title}' 모임에서 강퇴당했습니다.` : '모임에서 나갔습니다.');
                fetchGatherings(); 
            };
    ...
    useEffect(() => {
        if (meetingType === 'myMeetings') {
            const meetingToSelect = myGatherings.find(m => m.id === openChatId) || myGatherings[0] || null;
    ...
    const handleCreateGathering = async (gatheringData) => {
        try {
            ...
            showAlert(error.response?.data?.message || "모임 생성 중 오류가 발생했습니다.");
        }
    };

    const handleJoinLeave = async (gathering, isParticipant) => {
        if (!user) return;
        const endpoint = isParticipant ? `/gatherings/${gathering.id}/leave` : `/gatherings/${gathering.id}/join`;
        try {
            await apiClient.post(endpoint);
            fetchGatherings();
        } catch (error) {
            console.error(`모임 ${isParticipant ? '나가기' : '참여'} 오류:`, error);
            showAlert(error.response?.data?.message || "작업 중 오류가 발생했습니다.");
        }
    };
    
    const handleDelete = (gatheringId) => { setGatheringToDelete(gatheringId); };
    ...
    const calculateTimeRemaining = (deadline) => { if (!deadline) return null; const now = new Date(); const deadlineDate = new Date(deadline); const diff = deadlineDate - now; if (diff <= 0) return <span className="text-red-600">모집 마감</span>; const days = Math.floor(diff / (1000 * 60 * 60 * 24)); const hours = Math.floor((diff / (1000 * 60 * 60)) % 24); const minutes = Math.floor((diff / 1000 / 60) % 60); if (days > 0) return `${days}일 ${hours}시간 뒤 마감`; if (hours > 0) return `${hours}시간 ${minutes}분 뒤 마감`; if (minutes > 0) return `${minutes}분 뒤 마감`; return <span className="text-orange-500">마감 임박</span>; };
        ...
        <div className="min-h-screen bg-gray-50">
            <main className="py-8 max-w-6xl mx-auto px-4">
                <div className="text-center mb-8">
                    <div ref={helpRef} className="relative flex justify-center items-center gap-2"> <h1 className="text-4xl font-bold text-gray-800">번개모임</h1> <button onClick={() => setShowHelp(!showHelp)} className="text-gray-400 hover:text-gray-600 transition-colors"><i className="fa-solid fa-circle-question fa-lg"></i></button> {showHelp && ( <div className="absolute top-full mt-3 w-80 bg-white border border-gray-200 rounded-xl shadow-xl p-4 text-left z-20 animate-fadeIn"> <h4 className="font-bold text-md mb-2 text-gray-800"> 번개모임 사용 유의사항</h4> <ul className="space-y-1.5 text-sm text-gray-600 list-disc list-inside"> <li>안전한 만남을 위해 가급적 교내 또는 공공장소에서 만나세요.</li> <li>개인정보(연락처, 주소 등) 공유에 주의하세요.</li> <li>불쾌감을 주는 언행이나 행동은 삼가주세요.</li> </ul> <button onClick={() => setShowHelp(false)} className="absolute top-2 right-2 text-gray-400 hover:text-gray-600"><i className="fa-solid fa-times"></i></button> </div> )} </div>
                        ...
                        <aside className="w-1/3 border-r border-gray-200 flex flex-col">
                            ...
                            <div className="overflow-y-auto flex-1">
                                {myGatherings.length > 0 ? (
                                    myGatherings.map(m => (
                                        <div key={m.id} className={`w-full text-left p-4 hover:bg-gray-100 relative group ${selectedMeeting?.id === m.id ? 'bg-blue-50' : ''}`}>
                                            <button onClick={() => setOpenChatId(m.id)} className="w-full text-left">
                                            ...
                                            {user.id !== m.creator.id && !m.kickedUserIds.includes(user.id) && (
                                                <button onClick={() => handleJoinLeave(m, true)} className="absolute top-1/2 right-3 -translate-y-1/2 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition" title="모임 나가기">
                             ...
                             <div className="p-3 border-t bg-gray-50 text-xs text-gray-500">
                                 <p className="mb-2"><i className="fa-solid fa-circle-info mr-1.5"></i>모임 시간 전까지 연락 수단을 확보하세요. 설정한 시간이 되면 채팅방은 비활성화됩니다.</p>
                                 <p><i className="fa-solid fa-database mr-1.5"></i>모든 채팅 데이터는 안정적인 서비스 제공을 위해 30일간 보관 후 자동 삭제됩니다.</p>
                             </div>
                        </aside>
                        <ChatView user={user} meeting={selectedMeeting} socket={socket} onKickUser={handleKickUser} onLeaveMeeting={() => handleJoinLeave(selectedMeeting, true)} onAcknowledgeKick={handleAcknowledgeKick} />
                    </div>
                ) : (
                    <>
                        <div className="text-center mb-8">
                            <button onClick={() => meetingType === 'meeting' ? setShowCreateModal(true) : setShowCarpoolModal(true)} disabled={(meetingType === 'meeting' && isInMeeting) || (meetingType === 'carpool' && isInCarpool)} className="bg-gradient-to-r from-blue-500 to-purple-500 text-white px-8 py-4 rounded-xl font-semibold hover:shadow-lg transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed" title={(meetingType === 'meeting' && isInMeeting) ? "이미 참여 중인 '취미&약속' 모임이 있습니다." : (meetingType === 'carpool' && isInCarpool) ? "이미 참여 중인 '택시&카풀'이 있습니다." : "새 모임 만들기" }>
                            ...
                            <div className="bg-white rounded-xl shadow-sm p-4 mb-6">
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    {["전체", "점심", "술", "취미", "스터디", "운동", "게임"].map(tag => (
                                        <button key={tag} onClick={() => setActiveTag(tag)} className={`px-3 py-1.5 text-sm rounded-full font-semibold transition ${activeTag === tag ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>{tag}</button>
                                    ))}
                                </div>
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            {isLoading ? <div className="col-span-full text-center py-8"><div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div><p className="text-gray-500">모임을 불러오는 중...</p></div>
                             : filteredGatherings.length > 0 ? (
                                filteredGatherings.map(m => {
                                    const isParticipant = myGatherings.some(myM => myM.id === m.id);
                                    const isCreator = user ? m.creator.id === user.id : false;
                                            ...
                                            <div className="p-5 flex flex-col flex-grow">
                                                ...
                                                <div className="mt-auto pt-4 border-t">
                                                    ...
                                                    <div className="flex justify-between items-center">
                                                         <UserDisplay userTarget={{ id: m.creator.id, nickname: m.creator.nickname }} context={{ type: 'meeting', id: m.id }} ><span className="text-sm font-medium text-gray-500 cursor-pointer"><i className="fa-regular fa-user mr-1.5"></i>{m.creator.nickname}</span></UserDisplay>
                                                        <div className="flex items-center space-x-2">
                                                            ...
                                                            {user && !isCreator && <button onClick={() => handleJoinLeave(m, isParticipant)} disabled={(isFull && !isParticipant)} className={`px-3 py-1.5 rounded-md text-xs font-bold text-white transition ${ isParticipant ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600' } ${ (isFull && !isParticipant) ? 'cursor-not-allowed opacity-70' : ''}`}>{isParticipant ? '나가기' : '참여'}</button>}
            ...
            {gatheringToDelete && (<ConfirmModal message="정말로 이 모임을 삭제하시겠습니까?" onConfirm={executeDelete} onCancel={() => setGatheringToDelete(null)} />)}
