...
const CreateMeetingModal = ({ user, onCreate, show, onClose }) => {
  ...
  if (!show) return null;
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => onClose(false)}>
      <form onSubmit={handleSubmit} className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
...
export default function MeetingPage() {
  ...
  useEffect(() => {
    if (!authLoading && user) {
      const s = io("https://api.campuseat.shop/gatherings", { query: { userId: user.id } });
      setSocket(s);
