...
export default function TextEditor({ initialContent, onContentChange }) {
    ...
    const selectionRef = useRef(null); // ✅ 수정 4: 링크 삽입 시 선택 영역 저장을 위함
    ...
    // ✅ 수정 4: prompt 대신 모달을 사용하도록 로직 변경
    const openLinkModal = () => {
        // 모달을 열기 전에 현재 선택 영역을 저장
    ...
    const confirmInsertLink = (url) => {
        ...
        selectionRef.current = null; // 사용 후 초기화
    };
        ...
        for (let file of files) {
            if (file.type.startsWith('image/')) {
                ...
                try {
                    ...
                    // ✅ 수정 4: alert 대신 console.error 사용
