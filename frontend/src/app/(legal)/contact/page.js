...
export default function ContactPage() {
  ...
  // ✅ 2. fetchInquiries를 useCallback으로 감싸서 함수의 재생성을 방지합니다.
  ...
  }, [user]); // user가 변경될 때만 이 함수가 새로 만들어집니다.
  
  useEffect(() => {
    if (user && mode === 'myInquiries') {
      fetchInquiries();
    }
  // ✅ 3. 의존성 배열에 fetchInquiries를 추가하여 경고를 해결합니다.
  ...
  const handleInquiryClick = async (inquiryId) => {
    ...
    if (newOpenInquiryId && !answers[newOpenInquiryId]) {
      try {
        const response = await apiClient.get(`/inquiries/${inquiryId}/answers`);
  ...
  const handleSubmitInquiry = async () => {
    ...
    try {
      ...
      setAlertMessage(error.response?.data?.message || "문의 접수 중 오류가 발생했습니다.");
      setShowAlertModal(true);
    } finally {
