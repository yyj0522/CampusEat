...
export default function SignUpStep3({ formData, prev }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiClient.post('/auth/verify-code', {
        email: formData.email, // Use the main email from Step 1
        code: codeInput,
      });

      const { accessToken } = response.data;
      
      // Manually save the token. The AuthProvider will pick it up on the next page load.
  ...
  const handleSuccessModalClose = () => {
    ...
    // Force a full page reload to ensure AuthProvider re-evaluates the login state
